...
if __name__ == "__main__":
    # Parse a single instance index from CLI without interfering with Hydra overrides
    cli_parser = argparse.ArgumentParser(add_help=False)
    cli_parser.add_argument("--instance_index", type=int, default=None, help="Index (0-based) into test_instances.csv")
    cli_args, remaining_argv = cli_parser.parse_known_args()

    # Let Hydra consume the remaining args as usual
    ...
    # If an instance index is provided, force evaluation to only that index
    if cli_args.instance_index is not None:
        config.eval_instance_ids = [cli_args.instance_index]
    ...
    print('zql instances_to_run:', instances_to_run)
    print('zql list(range(m.NUM_EVAL_INSTANCES)):', list(range(m.NUM_EVAL_INSTANCES)))
